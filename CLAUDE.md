...
**Role-based programs (multiple per user, v1.2.0):** Programs in `managed_programs`: **BDR Training Program** (id `new-am-training`, kept for compat; core 5 + bdr-role + ongoing-development) and **AM Training Program** (id `am-training`; core 5 + am-role + ongoing-development). A user can hold MULTIPLE programs via the `user_programs(user_id, program_id)` join table (migration 016) — assign/remove with the multi-select in Admin → Manage Users. **New users start with NO program** (no auto-default); the old `profiles.program_id` is deprecated (kept only as an AuthContext fallback when `user_programs` is unavailable). `AuthContext` loads `User.programIds[]`; `CoursesContext.getProgramsForUser(programIds)` returns the user's programs (the old singular `getProgramForUser` is gone). Home shows one grouped "My Progress" timeline per assigned program (`ProgressTimeline`, replaced the old course grid + `ProgramCard`); gating (`useCourseLock`) unlocks a course if ANY assigned program grants it; a user with no programs sees a "no program assigned" state; certificates are **per program** (`/certificate/program/:programId`, one per completed program). Admins/leadership see a separate "All Courses" section. A third legacy program `new-hire-bdr-onboarding` exists at sort 3 — possible duplicate, Miguel to review/delete via Content Hub.
...
- **Version discipline**: `package.json` `version` is the single source of truth. The app injects it at build time via Vite `define` (`__APP_VERSION__` in `vite.config.ts`, typed in `src/globals.d.ts`, read by `src/version.ts`'s `APP_VERSION`, shown in Sidebar + Login footer). Never hand-edit `version.ts`. To ship a user-visible release: run `npm version patch|minor` (bumps package.json, commits, and creates the matching `vX.Y.Z` git tag in one step), add a `DevLog.tsx` changelog entry, then `git push --follow-tags`. Patch for fixes, minor for features. Infra-only deploys that don't change what users see keep the same version (no bump, no changelog).
