    ...
    >
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
        ...
        <div className="bg-slate-50 rounded-lg p-4 mb-4">
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-slate-900">{marketStats.annualValue}</span>
            <span className="text-sm text-slate-500">combined annual value of merchandise retailers need to liquidate</span>
          </div>
        </div>
        <p className="text-sm text-slate-600 leading-relaxed mb-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
        {salesChannels.map((channel) => (
          ...
          >
            <span className="text-slate-500 mt-0.5 shrink-0">{iconMap[channel.icon]}</span>
            <div>
      ...
      <div className="bg-blue-50 rounded-lg border border-blue-200 p-4">
        <p className="text-sm text-blue-800 font-medium mb-1">Why This Matters for Sales</p>
        <p className="text-sm text-blue-700">{marketStats.whyItMatters}</p>
      </div>
    </SectionWrapper>
  )
}
