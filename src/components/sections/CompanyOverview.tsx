  ...
  return (
    ...
    >
      ...
      <div className="bg-via-orange/10 rounded-xl border border-via-orange/30 p-6 mb-6">
        ...
        <p className="text-sm text-orange-200/80 italic leading-relaxed">
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          {coreValues.map((value) => (
            <div key={value.name} className="flex items-center gap-1.5">
              ...
              <span className="text-sm font-medium text-orange-200">{value.name}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {retailerPartners.map((partner) => (
          <span
            key={partner}
            className="px-3 py-1.5 bg-via-card border border-via-border rounded-lg text-sm font-medium text-slate-300"
          >
            {partner}
          </span>
        ))}
